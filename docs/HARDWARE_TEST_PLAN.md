...
| 4a | Filter row map (1/3) | `SETUP` → `→`×6 to P7 (MIDI filtering — **two fields only**: `<msg type> <bloc/merg>`; there is **no channel field**, filtering is per-type device-wide). Left → `CTR CHANGE`, Right → `BLOC`. Dump. **⚠️ P8/P9 beyond this page are INIT PRESET / INIT MEMORY — never press CTR STORE there.** | Prediction: filter-frame `0x0C → 0x00`. Working model: cell offset = `0x06 + 2 × (type index in the manual's list order — the same enum as the CMD types)`, which retro-explains T3's `0x08`/`0x0C` as NOTE ON / CTR CHANGE (the "channels" in T3's notes never existed on this page). |
| 4b | Filter row map (2/3) | SETUP P7: Left → `PROG CHANGE`, Right → `BLOC`. Dump. | Prediction: `0x0E → 0x00` — a never-captured type; the real test of the row map. |
| 4c | Filter row map (3/3, optional) | SETUP P7: Left → `NOTE OFF`, Right → `BLOC`. Dump. Also record the **full list + order** of types the left field cycles through. | Prediction: `0x06 → 0x00`; the recorded list pins every remaining row. |
