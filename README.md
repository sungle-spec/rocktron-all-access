...
- **Write All** — sends the current dump back, byte-by-byte at the manual's literal "1 byte every 15 ms" rate (~11 minutes for a full 43 KB dump). Live progress in the toast + server log. **Refuses to write a partial/corrupt dump** (validates the exact 145-frame multiset first). ⚠️ **Known issue:** on real hardware the last preset (PR120) comes back blank and a few global options are corrupt after a bulk restore. It's a device firmware quirk (device-to-device restore fails identically), not the editor. **Workaround: factory reset (SETUP P9 = code 230), then restore on top** — PR120 sticks. See [docs/HARDWARE_TEST_PLAN.md](docs/HARDWARE_TEST_PLAN.md).
